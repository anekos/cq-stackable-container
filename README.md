...
$ uv run stackable-container build --dividers-x 2 --dividers-y 1   # 3x2 に区切る
...
| `thickness` | 2.0 | 壁厚・床厚 |
| `dividers_x` | 0 | X を分割する仕切りの枚数（Y 方向に走る板） |
| `dividers_y` | 0 | Y を分割する仕切りの枚数（X 方向に走る板） |

既定値は `sample.stl` と同じ（仕切りなし）。それ以外の寸法は壁厚 `t` に対する比としてモジュール定数に
...
`--dividers-x` / `--dividers-y` で、壁厚と同じ厚みの板を等間隔に立てる。区画が等幅に
なるように配置するので、n 枚なら 1 区画は `(内寸 - n * t) / (n + 1)`（120/t=2 で 2 枚なら
37.33 mm）。入りきらない枚数を指定すると `ValueError`。

**仕切りは上端まで届かせてはいけない。** 上に積んだ箱の脚が内側に落ち込んでくるため。
落ち込む深さは段差の形状から決まる：上の箱は、S 字の下側円弧が下の箱の内リム
（`width/2 - t`）に触れたところで止まる。つまり後退量がちょうど `t` になる点で、

```
contact = acos(1 - clearance / blend)
depth   = LIP_HEIGHT - blend * (sin θ - sin contact)
```

既定値では 10.10 mm。仕切りの上端はここからさらに `clearance` 下げた
`height - depth - clearance`（= 69.50）にしている。

板の長さ（壁に突き当たる方向）は、外形より長く作ってから外形ソリッドで `intersect` して
切り落としている。板が届くべき位置は高さによって違うため：

| z | 外壁 | 内壁 | 板の端 |
| --- | --- | --- | --- |
| 5（脚） | ±57.6 | ±55.6 | 55.6〜57.6 |
| 20（本体） | ±60.0 | ±58.0 | 58.0〜60.0 |

内寸ちょうど（±58）で作ると脚のところで 0.4 mm 外にはみ出し、脚に合わせる（±57.6）と
本体側で内壁に届かない。大きめに作って切る方式なら、高さごとの正解を計算せずに済む。
...
5. 仕切りが指定されていれば `_dividers` の板を外形で切ってから union する。
